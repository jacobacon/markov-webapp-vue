  ...
  <v-container>
    ...
    <v-card hover>
      <v-container>
        ...
        <v-textarea v-model="training" placeholder="Enter Training Text"></v-textarea>
        <span style="color: red" v-if="!training">Enter  Some Training Text!</span>
      </v-container>
    </v-card>
    <v-card class="mt-3" hover>
      <v-container>
        <v-layout column>
          <h3>Generated Text</h3> <br>
      ...
      <v-container>
        <v-layout row justify-space-between>
          <span>
            ...
            <v-text-field placeholder="Enter Word" v-model="seed"></v-text-field>
          </span>
          <span>
            <v-label for="text-length">Text Length</v-label>
            <v-text-field placeholder="value" v-model="length" type="number" min="0" max="999" id="text-length"></v-text-field>
          </span>
          <v-btn color="green" round @click="train()">Generate Text</v-btn>
        </v-layout>
      </v-container>
    </v-card>
  </v-container>
  ...
  export default {
    ...
    methods: {
      ...
      generateText: function (length) {
        if (!this.seed) {
          this.seed = this.wordArray[Math.floor(Math.random() * (this.wordArray.length))]
        }
